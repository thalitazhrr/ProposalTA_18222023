...
Perbedaan utama antara klasifikasi multi-kelas dan biner adalah pada interpretasi elemen matriks kebingungan (\textit{confusion matrix}) dan agregasi metrik per-kelas. Untuk 38 kelas, matriks kebingungan berukuran 38×38, di mana setiap baris merepresentasikan kelas hasil prediksi dan setiap kolom merepresentasikan kelas sebenarnya. Prediksi yang benar terletak pada diagonal utama, sedangkan prediksi salah tersebar pada sel off-diagonal.\parencite{EvidentlyMultiClass} Pola sebaran kesalahan pada matriks kebingungan multi-kelas memberikan wawasan berharga tentang persamaan atau kebingungan antar kelas, misalnya prediksi kelas ``Penjualan-Bulanan'' yang sering disalahklasifikasikan sebagai ``Penjualan-Triwulanan'' menunjukkan kesamaan semantik yang perlu ditangani dengan aturan atau templat yang lebih jelas.

\paragraph*{Cara Menghitung TP, FP, FN untuk Setiap Kelas (One-vs-All):}

Untuk menghitung metrik per kelas-\(i\), masalah klasifikasi multi-kelas diubah menjadi masalah biner secara virtual melalui pendekatan \textit{one-vs-all}:\parencite{Grandini2020MultiClassMetrics}
...
\paragraph*{Agregasi Metrik: Macro-Average versus Micro-Average:}

Karena ada 38 kelas dalam sistem ini, diperlukan strategi agregasi untuk memperoleh metrik keseluruhan performa sistem. Dua pendekatan utama yang umum digunakan dalam klasifikasi multi-kelas adalah \textit{macro-averaging} dan \textit{micro-averaging}:\parencite{EvidentlyMultiClass}

\textbf{Macro-Averaging.} Pendekatan ini menghitung metrik (\textit{Precision}, \textit{Recall}, \textit{F1-Score}) untuk setiap kelas secara individual, kemudian mengambil rata-rata aritmatik dari 38 nilai metrik tersebut. Strategi ini memberikan bobot yang sama untuk setiap kelas, sehingga mengutamakan performa pada kelas minoritas dan mencegah kelas mayoritas mendominasi hasil evaluasi.\parencite{Grandini2020MultiClassMetrics}
...
\textbf{Weighted-Averaging.} Sebagai pendekatan komplementer, rata-rata tertimbang menghitung metrik per-kelas kemudian mengambil rata-rata yang diboboti oleh jumlah sampel di setiap kelas. Ini mengakomodasi keseimbangan antara kepedulian terhadap kelas minoritas dan representasi data yang ada.\parencite{Grandini2020MultiClassMetrics}

\paragraph*{Evaluasi dengan Tingkat Keyakinan:}

Sistem menggunakan mekanisme pemberian skor keyakinan (\textit{confidence scoring}) dalam rentang 0 hingga 1 untuk setiap prediksi klasifikasi intent. Tingkat keyakinan tertinggi menunjukkan kelas yang diprediksi oleh sistem. Untuk menentukan apakah prediksi dapat diterima atau perlu klarifikasi, ditetapkan \textit{threshold} pada nilai 0{,}70:\parencite{JATITTransformerIntent}
...
\paragraph*{Matriks Kebingungan Multi-Kelas 38×38:}
...
\paragraph*{Metrik yang Digunakan dalam Penelitian:}

Karena setiap kelas intent memiliki kepentingan strategis yang berbeda dalam konteks bisnis (contohnya: intent ``Prediksi-Churn-Pelanggan'' lebih kritis bagi keputusan strategis daripada ``Informasi-Umum''), penelitian ini menggunakan kombinasi metrik berikut:\parencite{Grandini2020MultiClassMetrics}
...
Berdasarkan studi industri dan best-practices untuk sistem klasifikasi intent dalam chatbot bisnis, target performa yang ditetapkan untuk penelitian ini adalah:\parencite{JATITTransformerIntent,Ozerova2022IntentModule}
