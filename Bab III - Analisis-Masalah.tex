...
Berikut adalah empat alternatif solusi utama yang telah diidentifikasi dan dianalisis berdasarkan studi literatur, praktik industri, dan kebutuhan fungsional dan non fungsional:
...
Tahap pemahaman data (\textit{data understanding}) merupakan fase kedua dalam metodologi CRISP-DM yang bertujuan untuk mengumpulkan data awal, mengidentifikasi karakteristik data, mendeteksi permasalahan kualitas data, serta menemukan wawasan awal yang dapat membentuk hipotesis terhadap informasi tersembunyi dalam data. Pada konteks pengembangan sistem \textit{Business Intelligence} berbasis \textit{chatbot}, tahap ini berfokus pada eksplorasi komprehensif terhadap lima domain data utama yang tersimpan dalam skema \texttt{USR\_RPT} pada \textit{Enterprise Data Warehouse}.

\subsection{Sumber Data dan Arsitektur Penyimpanan}

Sistem peramalan dan analisis performa berbasis \textit{chatbot} terhubung secara langsung dengan sumber data utama berupa \textit{Enterprise Data Warehouse} (EDW) yang menggunakan basis data Oracle pada server dengan skema \texttt{USR\_RPT}. Koneksi yang diatur menggunakan metode \textit{connection pooling} serta \textit{parameter binding} untuk menjamin efisiensi dan keamanan saat eksekusi kueri. Skema \texttt{USR\_RPT} berisi lima domain utama yang masing-masing memiliki fokus bisnis dan struktur data berbeda.

\begin{table}[h]
\centering
\caption{Ringkasan Domain Data pada Skema USR\_RPT}
\label{tab:ringkasan-domain}
\begin{tabular}{|p{3.5cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Domain Data} & \textbf{Jumlah Aturan Bisnis} & \textbf{Fokus Bisnis} \\
\hline
\texttt{BRIGHTAI\_SALES} & 12 & Analisis volume pesanan, kinerja kanal penjualan, dan penetrasi layanan pada berbagai wilayah operasional \\
\hline
\texttt{BRIGHTAI\_REVENUE} & 8 & Evaluasi pendapatan, profitabilitas pelanggan, dan distribusi pendapatan per wilayah dan segmen \\
\hline
\texttt{BRIGHTAI\_DAPROS} & 7 & Segmentasi dan profiling pelanggan berbasis data pemasangan, keaktifan digital, dan program loyalitas \\
\hline
\texttt{BRIGHTAI\_TARGET} & 5 & Pemantauan pencapaian target kinerja dan analisis kesenjangan terhadap target periode \\
\hline
\texttt{BRIGHTAI\_CT0\_NAL} & 6 & Identifikasi potensi \textit{churn} pelanggan dan strategi retensi melalui analisis siklus hidup layanan \\
\hline
\end{tabular}
\end{table}
...
\subsubsection{Domain BRIGHTAI\_SALES}

Domain \texttt{BRIGHTAI\_SALES} merupakan domain utama yang merepresentasikan informasi terkait volume permintaan, performa penjualan, serta penetrasi layanan pada berbagai tingkatan wilayah operasional. Tabel ini menyajikan struktur data yang komprehensif, mencakup dimensi geografis, waktu, produk, serta atribut pelanggan dan metrik performa layanan. Tabel~\ref{tab:spesifikasi-sales} menyajikan spesifikasi lengkap domain ini.
...
\hline
Tabel & \texttt{USR\_RPT.BRIGHTAI\_SALES} \\
\hline
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{USR\_RPT.BRIGHTAI\_REVENUE} \\
\hline
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{USR\_RPT.BRIGHTAI\_DAPROS} \\
\hline
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{USR\_RPT.BRIGHTAI\_TARGET} \\
\hline
...
\subsubsection{Domain BRIGHTAI\_CT0\_NAL}

Domain \texttt{BRIGHTAI\_CT0\_NAL} dirancang untuk menganalisis perilaku \textit{churn} pelanggan serta strategi retensi yang berkaitan dengan siklus hidup layanan. Domain ini memungkinkan pengukuran tingkat \textit{churn}, identifikasi pelanggan dengan risiko tinggi, serta pelacakan masa aktif layanan pelanggan. Struktur data dalam \texttt{BRIGHTAI\_CT0\_NAL} mencakup status pelanggan, metrik \textit{churn}, alasan perpindahan layanan, serta parameter durasi berlangganan. Tabel~\ref{tab:spesifikasi-churn} menyajikan spesifikasi lengkap domain ini.
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{USR\_RPT.BRIGHTAI\_CT0\_NAL} \\
\hline
...
\hline
BRIGHTAI\_SALES & 98.5 & 1.5 & Minimal & Teridentifikasi \\
\hline
BRIGHTAI\_REVENUE & 99.2 & 0.8 & Minimal & Teridentifikasi \\
\hline
BRIGHTAI\_DAPROS & 97.8 & 2.2 & Sedang & Teridentifikasi \\
\hline
BRIGHTAI\_TARGET & 99.0 & 1.0 & Minimal & Teridentifikasi \\
\hline
BRIGHTAI\_CT0\_NAL & 96.5 & 3.5 & Sedang & Teridentifikasi \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item Domain BRIGHTAI\_SALES menunjukkan pola musiman yang jelas dengan siklus tahunan
    \item Domain BRIGHTAI\_REVENUE memiliki tren pertumbuhan yang stabil dengan volatilitas sedang
    \item Domain BRIGHTAI\_CT0\_NAL menunjukkan fluktuasi musiman terkait periode kontrak pelanggan
\end{enumerate}
